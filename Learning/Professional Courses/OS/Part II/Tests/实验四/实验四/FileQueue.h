...
public:
    ...
    bool *GetSec(string filename,const FileQueue &q);
    ...
    void Traverse2() const ;	// 遍历队列
    Status DelQueue(string &e);				     // 出队操作
    Status GetHead(string &e) const;			     // 取队头操作
    bool verification(string filename,int flag);
...
//根据传进来的sec判断打开文件时输入的保护码是不是正确的，如果正确就返回true
{
    for (File *p = front->next; p != NULL; p = p->next){
