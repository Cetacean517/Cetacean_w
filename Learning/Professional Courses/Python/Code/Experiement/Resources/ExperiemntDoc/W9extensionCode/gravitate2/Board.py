...
class Board(tk.Canvas):
    ...
    def _move(self):
        ...
        while moved:
            moved = False
            for x in ripple_range(self.columns):
                for y in ripple_range(self.rows):
...
def ripple_range(limit):
    ...
    middle = limit // 2
    yield middle
    for low, high in zip(range(middle - 1, -1, -1),
                         range(middle + 1, limit + 1)):
        if low != middle and low >= 0:
            yield low
        if high != middle and high < limit:
            yield high
