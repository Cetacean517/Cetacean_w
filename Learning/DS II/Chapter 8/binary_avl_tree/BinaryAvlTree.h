...
//  平衡二叉树方法声明及重载编译系统默认方法声明:
	...
	int getHeight(BinAVLTreeNode<ElemType>* r) const;
	int getHeight() const;
...
template<class ElemType>
int BinaryAVLTree<ElemType>::getHeight(BinAVLTreeNode<ElemType> *r) const
{
	if (r == NULL)
	{
		return 0;
	...
	{
		return getHeight(r->leftChild) + 1;
	}
	else
	{
		return getHeight(r->rightChild) + 1;
	}
}

template<class ElemType>
int BinaryAVLTree<ElemType>::getHeight() const
{
	BinAVLTreeNode<ElemType>* r = root;
	return getHeight(r);
}
