...
### 浮点数的加减运算
#### 1. 对阶
- 求阶差（用阶码做差，利用补码化作加法实现）
     $$
   \Delta j = j_x - j_y = 
   \begin{cases}
   0, & \text{$j_x = j_y$}    \\[3ex]
   \gt 0, & \text{$j_x \gt j_y$  } \qquad y向x看齐 \quad S_y\rightarrow1, j_y+1 \quad 小阶向大阶看齐\\[3ex] 
   \lt 0, & \text{$j_x \lt j_y$}  \qquad x向y看齐 \quad S_x\rightarrow1, j_x+1 \quad 小阶向大阶看齐
   \end{cases}
   $$  
#### 2. 尾数求和
#### 3. 规格化
...
特例： 
S = $[-0.5]_补$ 不是规格化的数
S = -1 没有规格化的原码，但是有规格化的补码
- 规格化方式
  - 左规：尾数左移1位，阶码减1，直到数符和第一数位不同为止。（对于补码）
  - 右规：尾数右移1位，阶码加1。[尾数溢出（>1）时，需要右规。即尾数出现01.xx...x或10.xx...x时]
#### 舍入
在对阶和右规过程中，可能出现尾数末尾丢失，引起误差，需考虑舍入。
- 截断法（朝0舍入）
- 0舍1入
- 恒置1法
- 朝 $+\infty$ 舍入
