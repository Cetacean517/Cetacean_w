...
// 操作结果：返回顶点v1的相对于v2的下一个邻接点	，不存在则返回-1	 
{
	AdjListNetworkArc<WeightType> *p;
	if (v1 < 0 || v1 >= vexNum)
	...
	while (p != NULL && p->adjVex != v2)	//找到下一个v2所在的位置，用指针p记录
	    p = p->nextarc;
...
//template<class ElemType, class WeightType>
//bool AdjListDirNetwork<ElemType, WeightType>::DFS(int v1, int v2) //深度优先
//{
//	int w;
//	int flag = 0;
//	tag[v1] = VISITED;
//
//	for (w = FirstAdjVex(v1); w != -1; w = NextAdjVex(v1,w))
//	{
//		if (tag[w] == UNVISITED )
//		{
//			DFS(w, v2);
...
template<class ElemType, class WeightType>
bool AdjListDirNetwork<ElemType, WeightType>::BFS(int v1, int v2)	//广度优先
...
template<class ElemType, class WeightType>
void AdjListDirNetwork<ElemType, WeightType>::ClearTag()
{
