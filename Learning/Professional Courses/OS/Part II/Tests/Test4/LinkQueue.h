#ifndef PROJECT4_LINKQUEUE_H
#define PROJECT4_LINKQUEUE_H
...
public:
    ...
    bool Hav(ElemType &e, const LinkQueue<ElemType> &q);
    Status DelQueue(ElemType &e);				    // 出队操作
    Status GetHead(ElemType &e) const;			    // 取队头操作
    File *GetFile(ElemType &e, const LinkQueue<ElemType> &q);
...
//
template<class ElemType>
File *LinkQueue<ElemType>::GetFile(ElemType & e,const LinkQueue<ElemType> &q) {
//获得给用户指向的他的文件列表的头结点
...
//
template<class ElemType>
bool LinkQueue<ElemType>::Hav(ElemType & e,const LinkQueue<ElemType> &q) {
//判断是否有这个用户
    for (Node<ElemType> *p = q.front->next; p != NULL; p = p->next){
        if(p->data==e){
            return true;
...
//	否则插入元素e为新的队尾，返回SUCCESS
{
    Node<ElemType> *p;
    File *file = new File();
    p = new Node<ElemType>(e,NULL,file);	        // 生成一个新结点
    if (p) {
