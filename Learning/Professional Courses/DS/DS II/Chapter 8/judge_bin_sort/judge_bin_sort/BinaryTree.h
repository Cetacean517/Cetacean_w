...
bool BinaryTree<ElemType>::JudgeBinTreeSort(BinTreeNode<ElemType>* r,ElemType &m)const
{
	int flag = true;
	if (r) {
		flag = JudgeBinTreeSort(r->leftChild,m);	// 首先遍历r的左子树
		if (!m || r->data < m) {
			return false;
		}
		m = r->data;
		flag = JudgeBinTreeSort(r->rightChild,m);	// 最后遍历r的右子树
	}
	return flag;
}
...
{
	LinkQueue <BinTreeNode<ElemType>* > q;	//定义一个队列queue
	BinTreeNode <ElemType>* p;
	int wmax = 0, cur = 0, beh;				//wmax为最大的宽度，cur是当前层数的结点数，beh是下一层的结点数
	if (root != NULL)						//非空树的时候，根结点入队
	{
		q.EnQueue(root);
		cur = 1;
		wmax = 1;
	}
	while (!q.IsEmpty())					//非空树的时候，逐层遍历计算宽度
	{
		beh = 0;							//清零
		for (int i = 0; i < cur; i++)
		{
			q.DelQueue(p);						//出队，并用p返回出队结点的值
			if (p->leftChild != NULL)
			{
...
//	以r为根的二叉树
{
	if (inLeft > inRight)	// 二叉树无结点,空二叉树
		...
		int mid = inLeft;
		while (in[mid] != pre[preLeft]) 	// 查找先序遍历中根结点在中序遍历中的位置，数组下标
			mid++;
